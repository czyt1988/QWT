...
/**
 * @brief 返回当前可用于操作的 X 轴。
 *
 * 选择策略（按优先级递减）：
 * 1. 轴必须可见（isAxisVisible() == true）；
 * 2. 若 XBottom 与 XTop 均可见，优先返回 XBottom；
 * 3. 若仅一条 X 轴可见，返回该轴；
 * 4. 若两条 X 轴均不可见，返回默认的 QwtAxis::XBottom（调用方可按需再次检查可见性）。
 *
 * @return 选中的 X 轴 ID
 */
QwtAxisId QwtPlot::validXAxisId() const
{
...
/**
 * @brief 返回当前可用于操作的 Y 轴。
 *
 * 选择策略（按优先级递减）：
 * 1. 轴必须可见；
 * 2. 若 YLeft 与 YRight 均可见，优先返回 YLeft；
 * 3. 若仅一条 Y 轴可见，返回该轴；
 * 4. 若两条 Y 轴均不可见，返回默认的 QwtAxis::YLeft。
 *
 * @return 选中的 Y 轴 ID
 */
QwtAxisId QwtPlot::validYAxisId() const
{
